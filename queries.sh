...
echo "$($PSQL "SELECT DISTINCT(t.name) FROM games g FULL JOIN teams t ON g.winner_id=t.team_id OR g.opponent_id=t.team_id WHERE (t.name Like 'Co%') ORDER BY t.name")" 
